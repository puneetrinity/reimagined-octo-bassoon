...
class ABTestManager:
    ...
    def record_result(self, result: ExperimentResult) -> None:
        ...
        if result.user_id in self.user_assignments:
            ...
            user.avg_response_time = ((user.avg_response_time * (n-1)) + result.response_time) / n
            user.success_rate = ((user.success_rate * (n-1)) + (1.0 if result.success else 0.0)) / n
        ...
        # Update running averages
        arm_stat["avg_response_time"] = ((arm_stat["avg_response_time"] * (n-1)) + result.response_time) / n
        arm_stat["success_rate"] = ((arm_stat["success_rate"] * (n-1)) + (1.0 if result.success else 0.0)) / n
        arm_stat["avg_cost"] = ((arm_stat["avg_cost"] * (n-1)) + result.cost_usd) / n
        
        if result.user_feedback is not None:
            arm_stat["satisfaction_score"] = ((arm_stat["satisfaction_score"] * (n-1)) + result.user_feedback) / n
        
        if result.conversion:
            arm_stat["conversion_rate"] = ((arm_stat["conversion_rate"] * (n-1)) + 1.0) / n
    ...
    def _check_stopping_conditions(self) -> None:
        ...
        total_requests = sum(arm_stat["requests"] for arm_stat in self.arm_stats.values())
        ...
        if significance_result["significant"]:
            self._stop_experiment(f"statistical_significance_reached: {significance_result['reason']}")
            return
        ...
        if (bandit_performance["requests"] > 100 and 
            baseline_performance["requests"] > 100 and
            bandit_performance["success_rate"] < baseline_performance["success_rate"] * 0.9):
    ...
    def _test_statistical_significance(self) -> Dict:
        ...
        response_time_diff = abs(bandit_stats["avg_response_time"] - baseline_stats["avg_response_time"])
        response_time_baseline = baseline_stats["avg_response_time"]
        
        success_rate_diff = bandit_stats["success_rate"] - baseline_stats["success_rate"]
        cost_diff = baseline_stats["avg_cost"] - bandit_stats["avg_cost"]  # Positive = bandit cheaper
        
        # Check for meaningful improvements
        meaningful_improvement = (
            success_rate_diff > self.min_effect_size or  # 5% better success rate
            cost_diff > self.min_effect_size or          # 5% cost reduction
            (response_time_diff / response_time_baseline) > self.min_effect_size  # 5% speed improvement
        )
        
        if meaningful_improvement:
            return {
                "significant": True,
                "reason": f"meaningful_improvement_detected",
                "improvements": {
                    "success_rate_diff": success_rate_diff,
                    "cost_diff": cost_diff,
                    "response_time_improvement": response_time_diff / response_time_baseline
                }
            }
    ...
    def get_experiment_status(self) -> Dict:
        ...
        status = {
            ...
            "progress": {
                ...
                "time_progress": (elapsed_time / (24 * 3600)) / self.max_experiment_days
            },
            "arm_performance": {}
        }
