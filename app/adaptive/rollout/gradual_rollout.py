...
class GradualRolloutManager:
    ...
    def record_request_metrics(
        ...
        if self.current_stage not in self.stage_metrics:
            self.stage_metrics[self.current_stage] = RolloutMetrics(
                ...
                baseline_response_time=self.baseline_metrics.get("response_time", response_time),
        ...
        metrics.error_rate = ((metrics.error_rate * (n-1)) + (0.0 if success else 1.0)) / n
        ...
        metrics.bandit_confidence = ((metrics.bandit_confidence * (n-1)) + bandit_confidence) / n
    ...
    def _update_baseline_metrics(self, success: bool, response_time: float, cost: float) -> None:
        ...
        self.baseline_metrics["error_rate"] = ((self.baseline_metrics["error_rate"] * (n-1)) + (0.0 if success else 1.0)) / n
        self.baseline_metrics["response_time"] = ((self.baseline_metrics["response_time"] * (n-1)) + response_time) / n
        self.baseline_metrics["cost"] = ((self.baseline_metrics["cost"] * (n-1)) + cost) / n
    ...
    def _check_emergency_conditions(self, metrics: RolloutMetrics) -> None:
        ...
        if metrics.error_rate > self.emergency_rollback_threshold:
            emergency_conditions.append(f"error_rate_too_high: {metrics.error_rate:.3f}")
        
        if metrics.response_time_degradation > 0.5:  # 50% degradation
            emergency_conditions.append(f"response_time_degraded: {metrics.response_time_degradation:.3f}")
    ...
    def _check_stage_advancement(self, metrics: RolloutMetrics) -> None:
        ...
        request_requirement_met = metrics.requests_in_stage >= config.min_requests_in_stage
        
        # Check performance requirements
        error_rate_ok = metrics.error_rate <= config.max_error_rate
        response_time_ok = metrics.response_time_degradation <= config.max_response_time_degradation
        confidence_ok = metrics.bandit_confidence >= config.min_confidence_threshold
        
        # All requirements must be met
        if (time_requirement_met and request_requirement_met and 
            error_rate_ok and response_time_ok and confidence_ok):
    ...
    def _consider_rollback(self, metrics: RolloutMetrics, config: RolloutConfig) -> None:
        ...
        if metrics.response_time_degradation > config.max_response_time_degradation * 2:
            rollback_reasons.append(f"response_time_degraded: {metrics.response_time_degradation:.3f}")
    ...
    def _is_stage_ready_for_advancement(self) -> bool:
        ...
        return (
            current_metrics.hours_in_stage >= config.min_hours_in_stage and
            current_metrics.requests_in_stage >= config.min_requests_in_stage and
            current_metrics.error_rate <= config.max_error_rate and
            current_metrics.response_time_degradation <= config.max_response_time_degradation and
            current_metrics.bandit_confidence >= config.min_confidence_threshold
        )
