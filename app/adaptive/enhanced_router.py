...
from app.adaptive.rollout.gradual_rollout import GradualRolloutManager, RolloutStage
...
class EnhancedAdaptiveRouter:
    ...
    def __init__(
        ...
        if enable_adaptive:
            ...
            # Advanced monitoring and alerting
            self.monitor = AdaptiveMonitor(retention_hours=48)  # 48h retention for Week 2
            ...
            if enable_gradual_rollout:
                self.rollout_manager = GradualRolloutManager(
                    rollout_name="adaptive_routing_v2",
                    auto_advance=True
                )
            else:
    ...
    async def _execute_baseline_route(self, query: str, state: GraphState, request_id: str) -> Any:
    ...
    async def _execute_bandit_route(self, query: str, state: GraphState, request_id: str) -> Any:
    ...
    async def _execute_control_route(self, query: str, state: GraphState, request_id: str) -> Any:
        """Execute control routing (random selection for calibration)"""
        import random
        
        # Random selection between available routes
        available_routes = ["fast_chat", "search_augmented", "api_fallback", "hybrid_mode"]
    ...
    def _extract_routing_context(self, query: str, state: GraphState) -> Dict[str, Any]:
        ...
        complex_keywords = ["analyze", "compare", "research", "detailed", "comprehensive"]
        ...
        context.update({
            ...
            "has_search_keywords": any(keyword in query.lower() for keyword in search_keywords),
    ...
    async def _record_request_completion(
        ...
        if self.enable_ab_testing and self.current_experiment:
            experiment_result = ExperimentResult(
                experiment_id=self.current_experiment.experiment_id,
                user_id=user_id,
                arm=ExperimentArm(routing_arm) if routing_arm in ["baseline", "bandit", "control"] else ExperimentArm.BASELINE,
                request_id=request_id,
                timestamp=time.time(),
                response_time=response_time,
                success=success,
        ...
        if success and routing_arm == "bandit" and self.enable_adaptive:
            ...
            reward_data = self.reward_calculator.calculate_enhanced_reward(enhanced_metrics)
    ...
    def _estimate_request_cost(self, routing_arm: str, query: str, success: bool) -> float:
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "request_stats": {
                ...
                "avg_cost_per_request": self.total_cost_usd / max(1, self.request_count)
            }
        }
