...
from app.monitoring.system_metrics import get_system_metrics_collector, get_cache_metrics_tracker
from app.monitoring.cost_tracker import get_cost_tracker, CostCategory
...
class Week2AdaptiveRouter(AdaptiveIntelligentRouter):
    ...
    async def route_query(self, query: str, state: GraphState) -> Any:
        ...
        try:
            if use_bandit:
                ...
                result = await self._execute_enhanced_bandit_routing(query, state, start_time)
                routing_type = "bandit"
            else:
    ...
    def _extract_enhanced_context(self, query: str, state: GraphState) -> Dict[str, Any]:
    ...
    def _calculate_query_complexity(self, query: str) -> float:
        ...
        complexity_indicators = {
            ...
            "question_words": min(1.0, sum(1 for word in ["what", "how", "why", "when", "where"] if word in query.lower()) / 3),
            "complex_keywords": min(1.0, sum(1 for keyword in ["analyze", "compare", "detailed", "comprehensive"] if keyword in query.lower()) / 2)
        }
    ...
    async def _update_bandit_with_enhanced_reward(
        ...
        actual_cost = self._track_route_cost(selected_arm, query, result, user_id, request_id)
        ...
        reward_data = self.enhanced_reward_calculator.calculate_enhanced_reward(enhanced_metrics)
    ...
    def _track_route_cost(self, arm: str, query: str, result: Any, user_id: str = None, request_id: str = None) -> float:
    ...
    def _estimate_api_calls(self, arm: str) -> int:
        ...
        return {"fast_chat": 0, "search_augmented": 2, "api_fallback": 3, "hybrid_mode": 1}.get(arm, 1)
    ...
    async def _record_enhanced_metrics(
        ...
        if hasattr(self, 'cost_tracker') and routing_type != "baseline":
            ...
            estimated_cost = self._track_route_cost(routing_type, query, result or {}, user_id)
        else:
    ...
    async def _check_safety_conditions(self) -> None:
        ...
        bandit_error_rate = 1.0 - (self.bandit_metrics["success_count"] / self.bandit_metrics["requests"])
        
        # Safety check: error rate
        if bandit_error_rate > self.max_error_rate:
            await self._emergency_rollback(f"Error rate too high: {bandit_error_rate:.3f}")
            return
        
        # Safety check: cost increase (if we have baseline data)
        if self.baseline_metrics["requests"] > 10:
            baseline_avg_cost = self.baseline_metrics["total_cost"] / self.baseline_metrics["requests"]
            bandit_avg_cost = self.bandit_metrics["total_cost"] / self.bandit_metrics["requests"]
            
            cost_increase = (bandit_avg_cost - baseline_avg_cost) / baseline_avg_cost
            if cost_increase > self.max_cost_increase:
                await self._emergency_rollback(f"Cost increase too high: {cost_increase:.3f}")
                return
    ...
    def increase_rollout_percentage(self, new_percentage: float) -> bool:
        ...
        if self.bandit_metrics["requests"] > 50:
            error_rate = 1.0 - (self.bandit_metrics["success_count"] / self.bandit_metrics["requests"])
            if error_rate > self.max_error_rate:
                logger.warning("cannot_increase_rollout_high_error_rate", error_rate=error_rate)
                return False
    ...
    def _store_adaptive_metrics_async(self, 
                                     routing_arm: str,
                                     success: bool, 
                                     response_time: float,
                                     cost_usd: float,
        ...
        def store_adaptive_metrics():
            try:
                ...
                if clickhouse_manager:
                    asyncio.create_task(clickhouse_manager.record_adaptive_metrics(
                ...
                logger.warning("failed_to_store_adaptive_metrics_in_clickhouse", error=str(e))
    ...
    def get_week2_status(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'cost_tracker'):
            ...
            status["budget_status"] = self.enhanced_reward_calculator.get_budget_status()
    ...
    def _get_performance_comparison(self) -> Dict[str, Any]:
        ...
        if self.baseline_metrics["requests"] > 0 and self.bandit_metrics["requests"] > 0:
            baseline_avg_time = self.baseline_metrics["total_response_time"] / self.baseline_metrics["requests"]
            bandit_avg_time = self.bandit_metrics["total_response_time"] / self.bandit_metrics["requests"]
            
            baseline_success_rate = self.baseline_metrics["success_count"] / self.baseline_metrics["requests"]
            bandit_success_rate = self.bandit_metrics["success_count"] / self.bandit_metrics["requests"]
            ...
            comparison = {
                ...
                "improvements": {
                    "response_time_pct": ((baseline_avg_time - bandit_avg_time) / baseline_avg_time) * 100,
                    "success_rate_pct": (bandit_success_rate - baseline_success_rate) * 100,
                    "cost_change_pct": ((bandit_avg_cost - baseline_avg_cost) / baseline_avg_cost) * 100
                }
            }
    ...
    def _get_safety_status(self) -> Dict[str, Any]:
        ...
        if self.baseline_metrics["requests"] > 0:
            ...
            checks.append({
                ...
                "status": "pass" if cost_increase <= self.max_cost_increase else "fail"
            })
