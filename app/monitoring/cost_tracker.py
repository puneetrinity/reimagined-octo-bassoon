...
class RealTimeCostTracker:
    ...
    def __init__(self, monthly_budget_usd: float = 50.0):
        ...
        logger.info("real_time_cost_tracker_initialized", 
                   monthly_budget=monthly_budget_usd)
    
    def record_local_inference_cost(self, 
                                   model_name: str, 
                                   tokens: int,
                                   user_id: str = None,
        ...
        logger.debug("local_inference_cost_recorded",
                    model=model_name, tokens=tokens, cost=cost)
    ...
    def record_external_api_cost(self, 
        ...
        logger.debug("external_api_cost_recorded",
                    provider=provider, model=model, tokens=tokens, cost=cost)
        
        return cost
    
    def record_search_api_cost(self, 
                              provider: str,
                              query: str,
                              results_count: int,
                              user_id: str = None,
        ...
        logger.debug("search_api_cost_recorded",
                    provider=provider, results=results_count, cost=cost)
        
        return cost
    
    def record_content_api_cost(self, 
                               provider: str,
                               url: str,
                               content_size_kb: int,
                               user_id: str = None,
        ...
        logger.debug("content_api_cost_recorded",
                    provider=provider, size_kb=content_size_kb, cost=cost)
        
        return cost
    
    def record_infrastructure_cost(self, 
                                  operation_type: str,
                                  quantity: float,
    ...
    def _update_api_usage(self, provider: str, endpoint: str, cost: float, tokens: int = 0):
        """Update API usage metrics"""
        key = f"{provider}:{endpoint}"
        
        if key not in self.api_usage:
            self.api_usage[key] = APIUsageMetrics(
                provider=provider,
                endpoint=endpoint
            )
    ...
    def _store_cost_event_async(self, event: CostEvent, model: str = None, tokens: int = None):
        ...
        def store_cost_event():
            try:
                ...
                if clickhouse_manager:
                    ...
                    asyncio.create_task(clickhouse_manager.record_cost_event(
    ...
    def get_current_spend(self, hours: int = 24) -> float:
        ...
        return sum(
            event.cost_usd for event in self.cost_events
    ...
    def get_budget_status(self) -> Dict[str, any]:
        ...
        if current_day > 0 and daily_spend > 0:
            projected_monthly = (monthly_spend + (daily_spend * (days_in_month - current_day)))
        else:
    ...
    def get_cost_breakdown(self, hours: int = 24) -> Dict[str, any]:
        ...
        for category in breakdown:
            if total_cost > 0:
                breakdown[category]["percentage"] = (breakdown[category]["total_cost"] / total_cost) * 100
            else:
    ...
    def get_user_costs(self, user_id: str, hours: int = 24) -> Dict[str, any]:
        ...
        return {
            ...
            "cost_by_category": {
                category.value: sum(e.cost_usd for e in user_events if e.category == category)
                for category in CostCategory
            }
        }
    ...
    def optimize_costs(self) -> List[str]:
        ...
        if budget_status["budget_utilization"] > 0.8:
            recommendations.append("Budget utilization is high - consider optimizing expensive operations")
        
        # Expensive external APIs
        external_cost = cost_breakdown["breakdown"].get("external_api", {}).get("total_cost", 0)
        total_cost = cost_breakdown["total_cost"]
        
        if total_cost > 0 and (external_cost / total_cost) > 0.6:
            recommendations.append("External API costs are high - consider using local models more")
        
        # High search costs
        search_cost = cost_breakdown["breakdown"].get("search_api", {}).get("total_cost", 0)
        if total_cost > 0 and (search_cost / total_cost) > 0.3:
            recommendations.append("Search API costs are high - implement better caching")
        
        # Low local inference usage
        local_cost = cost_breakdown["breakdown"].get("local_inference", {}).get("total_cost", 0)
        if total_cost > 0 and (local_cost / total_cost) < 0.3:
            recommendations.append("Consider increasing local model usage for cost savings")
