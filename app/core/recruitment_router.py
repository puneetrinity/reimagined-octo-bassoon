...
class RecruitmentModelRouter(ModelRouter):
    ...
    def __init__(self):
        ...
        logger.info("RecruitmentModelRouter initialized with recruitment-specific configurations")
    ...
    def analyze_recruitment_task(self, task_description: str, content: str = "") -> str:
        ...
        resume_indicators = [
            "parse resume", "extract resume", "resume data", "cv parsing",
            "extract skills", "work experience", "education", "contact info"
        ]
        ...
        bias_indicators = [
            "bias check", "inclusive language", "discrimination", "bias detection",
            "job description review", "jd bias", "inclusive hiring", "fair language"
        ]
        ...
        matching_indicators = [
            "match candidate", "persona fingerprint", "skill matching", "candidate fit",
            "compare skills", "analyze fit", "matching score", "compatibility"
        ]
        ...
        script_indicators = [
            "recruiter script", "conversation script", "interview prep", "outreach message",
            "personalized message", "recruitment email", "candidate outreach"
        ]
        if any(indicator in task_lower for indicator in script_indicators):
            logger.debug(f"Identified script generation task: {task_description[:50]}...")
            return "conversational_script_generation"
        
        # Report generation indicators
        report_indicators = [
            "generate report", "summary report", "candidate summary", "assessment report",
            "score report", "brief summary", "quick summary"
        ]
        if any(indicator in task_lower for indicator in report_indicators):
            logger.debug(f"Identified report generation task: {task_description[:50]}...")
    ...
    def _analyze_content_type(self, content: str) -> str:
        ...
        resume_markers = [
            "experience:", "education:", "skills:", "contact:", "@", "phone:",
        ...
        jd_markers = [
            ...
            "we are looking for", "candidate should", "required skills"
        ]
        ...
        comparison_markers = [
            "compare", "match", "vs", "versus", "candidate a", "candidate b",
            "persona:", "fingerprint:", "score:"
        ]
    ...
    def get_recruitment_model_config(self, task_type: str, content_complexity: Optional[str] = None) -> Dict[str, Any]:
    ...
    def optimize_for_batch_processing(self, task_types: List[str]) -> Dict[str, Any]:
        ...
        total_memory = sum(memory_manager.get_memory_requirement(model) for model in required_models)
        ...
        if total_memory <= 20:  # Can fit all models
            ...
            for model in required_models:
                ...
                high_priority_tasks = [
                    task for task, config in task_configs.items() 
    ...
    def get_cache_strategy(self, task_type: str) -> Dict[str, Any]:
        ...
        return task_specific_cache.get(task_type, {
            "ttl": base_strategy,
            "should_cache": True,
            "cache_key_includes": ["content_hash"]
        })
