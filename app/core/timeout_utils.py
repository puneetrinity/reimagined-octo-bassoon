    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            for arg in args:
                if hasattr(arg, 'messages') and arg.messages:
                    ...
                    for msg in reversed(arg.messages):
                        ...
                        if word_count > 50 or any(indicator in content.lower() for indicator in 
                                                ['research', 'analyze', 'comprehensive', 'detailed']):
            ...
            try:
                ...
                logger.warning(f"Adaptive timeout ({timeout}s) exceeded for {func.__name__}")
                return {
                    "error": "Request timed out", 
                    "timeout": timeout,
