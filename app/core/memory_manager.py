...
from app.core.config import MODEL_MEMORY_REQUIREMENTS, A5000_CONFIG, PRIORITY_TIERS
...
class A5000MemoryManager:
    ...
    def __init__(self):
        ...
        logger.info(f"A5000MemoryManager initialized with {self.config['available_vram_gb']}GB available VRAM")
    ...
    def can_fit_model(self, model_name: str, required_models: Optional[List[str]] = None) -> bool:
    ...
    def _can_make_room(self, required_memory: float, protected_models: List[str]) -> bool:
        ...
        for model_name, info in self.loaded_models.items():
            if (model_name not in protected_models and 
                info.priority_tier != "T0" and 
                info.status == "loaded"):
    ...
    async def ensure_model_loaded(self, model_name: str, required_models: Optional[List[str]] = None) -> bool:
    ...
    async def _load_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Loading model {model_name} ({required_memory}GB, {priority_tier})")
    ...
    async def _free_memory_for_model(self, model_name: str, protected_models: List[str]):
        ...
        for loaded_model, info in self.loaded_models.items():
            if (loaded_model not in protected_models and 
                loaded_model != model_name and
                info.priority_tier != "T0" and 
                info.status == "loaded"):
    ...
    async def _unload_model(self, model_name: str):
        ...
        try:
            ...
            logger.info(f"Model {model_name} unloaded (Memory usage: {self.current_usage_gb:.1f}GB)")
    ...
    def get_memory_stats(self) -> Dict:
        ...
        loaded_count = sum(1 for info in self.loaded_models.values() if info.status == "loaded")
        ...
        return {
            ...
            "free_memory_gb": round(self.config["available_vram_gb"] - self.current_usage_gb, 1),
            "memory_utilization": round((self.current_usage_gb / self.config["available_vram_gb"]) * 100, 1),
    ...
    def recommend_models_for_workflow(self, workflow_models: List[str]) -> List[str]:
        ...
        total_memory_needed = sum(self.get_memory_requirement(model) for model in workflow_models)
        ...
        if total_memory_needed <= self.config["available_vram_gb"]:
            ...
            # Add T1 models
            for model in workflow_models:
                if self.get_model_priority_tier(model) == "T1" and model not in priority_order:
                    priority_order.append(model)
            
            # Add remaining models until memory limit
            current_memory = sum(self.get_memory_requirement(model) for model in priority_order)
            for model in workflow_models:
                if model not in priority_order:
                    model_memory = self.get_memory_requirement(model)
                    if current_memory + model_memory <= self.config["available_vram_gb"]:
                        priority_order.append(model)
